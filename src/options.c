...
# if defined(MICRO) || defined(WIN32) || defined(GL_GRAPHICS) || defined(SDL_GRAPHICS)
...
initoptions()
{
	...
	flags.initrole  = ROLE_NONE;  /* -AJA- removed the -1 */
...
#if defined(GL_GRAPHICS) || defined(SDL_GRAPHICS)
	/* -AJA- glHack support.  Needs to happen after main config
	 *       file has been read.
	 */
	opts = getenv("GLHACKOPTIONS");
	if (opts)
	{
