...
# else
#  if defined(WIN32)
#define SAVESIZE	(PL_NSIZ + 40)	/* username-player.glHack-saved-game */
#  else
...
#  if defined(WIN32)
	...
	Sprintf(SAVEF, "%s.glHack-saved-game", encodedfnamebuf);
#  else
...
# ifdef VMS
	if ((fp = fopenp(fqname("glhackini", CONFIGPREFIX, 0), "r"))
								!= (FILE *)0) {
		configfile = "glhackini";
		return(fp);
	}
	if ((fp = fopenp("sys$login:glhack.ini", "r")) != (FILE *)0) {
		configfile = "glhack.ini";
		return(fp);
	}

	envp = nh_getenv("HOME");
	if (!envp)
		Strcpy(tmp_config, "glHack.cnf");
	else
		Sprintf(tmp_config, "%s%s", envp, "glHack.cnf");
	if ((fp = fopenp(tmp_config, "r")) != (FILE *)0)
		return(fp);
# else	/* should be only UNIX left */
	envp = nh_getenv("HOME");
	if (!envp)
		Strcpy(tmp_config, ".glhackrc");
	else
		Sprintf(tmp_config, "%s/%s", envp, ".glhackrc");
	if ((fp = fopenp(tmp_config, "r")) != (FILE *)0)
		return(fp);
...
/* AJA: added this to support writing back config in glHack.  Should
 *      return NULL if write-back is not available (e.g. for security
 *      reasons).  The return buffer is in static storage.
 */
const char *get_config_write_filename(void)
{
#ifdef UNIX
       static char tmp_config[BUFSZ];

       char *envp = nh_getenv("HOME");
       if (envp)
       {
		Sprintf(tmp_config, "%s/%s", envp, ".glhackrc");
		return tmp_config;
       ...
       return NULL;
#else
  //#error "Where to write config file for this platform ??"
#endif
}
