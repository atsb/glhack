...
/* whether running in 3D or 2D mode (GL only) */
int sdlgl_3d = 0;
...
#endif
#ifdef GL3D_GRAPHICS
static void Sdlgl_init_nhwindows_gl3d(int *argcp, char **argv);
#endif
#ifdef SDL_GRAPHICS
...
/* Interface definition, Real 3D version. Only difference is the name
 * field and the init function.
 */
#ifdef GL3D_GRAPHICS
struct window_procs sdlgl_3d_procs = 
{
    "GL3D", SDLGL_WC_FLAGS,
    0L,
    Sdlgl_init_nhwindows_gl3d,
    Sdlgl_player_selection, Sdlgl_askname,
    Sdlgl_get_nh_event,
    Sdlgl_exit_nhwindows,
    Sdlgl_suspend_nhwindows, Sdlgl_resume_nhwindows,
...
static void sdlgl_do_init_nhwindows(int *argcp, char **argv)
{
  ...
  else
  {
    ...
    SDL_GL_SetAttribute(SDL_GL_DEPTH_SIZE,   sdlgl_3d ? 16 : 0);
  ...
  if (sdlgl_3d)
    SDL_WarpMouse(sdlgl_width  / 3, sdlgl_height / 3);
  else
...
#ifdef GL3D_GRAPHICS
void Sdlgl_init_nhwindows_gl3d(int *argcp, char **argv)
{
  sdlgl_software = 0;
  sdlgl_3d = 1;

  /* There is no 3d-specific rendering procs, as the 2d stuff is still
   * required to draw all the text windows.  We just have check the
   * `sdlgl_3d' variable at various places (especially when drawing
   * the map window).
   */
  sdlgl_rend_procs = sdlgl_hardw_rend_procs;
