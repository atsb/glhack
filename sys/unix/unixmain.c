...
#define MAC_PATH_VALUE ".app/Contents/MacOS/"
	...
	if ((fd = restore_saved_game()) >= 0) {
		...
		if (discover || wizard) {
			if(ysn("Do you want to keep the save file?") == 'n')
...
char *argv[];
{
	...
	while(argc > 1 && argv[1][0] == '-'){
		...
		case 'p': /* profession (role) */
			if (argv[0][2]) {
			    if ((i = str2role(&argv[0][2])) != ROLE_NONE)
			    	flags.initrole = i;
			} else if (argc > 1) {
				argc--;
				argv++;
			    if ((i = str2role(argv[0])) != ROLE_NONE)
			    	flags.initrole = i;
			}
			break;
		case 'r': /* race */
			if (argv[0][2]) {
			    if ((i = str2race(&argv[0][2])) != ROLE_NONE)
			    	flags.initrace = i;
			} else if (argc > 1) {
				argc--;
				argv++;
			    if ((i = str2race(argv[0])) != ROLE_NONE)
			    	flags.initrace = i;
			}
			break;
		case '@':
